...
def func_fqb(img):
    ...
    date = date.slice(0, 4)
    return img.select('LC_Type1').clip(aoi) \
    .set('year', date)
...
print(greennessColl, 'Greenness Image Collection'.getInfo())
print(precipColl, 'Precip Image Collection'.getInfo())
...
print('Module output of residuals', residualColl.getInfo())

resids = residualColl.first()
res1 = resids.select(['residual'])
print(res1.getInfo(), 'residual image'.getInfo())
Map.addLayer(res1, {
    'min': -0.2,
    'max': 0.2,
...
print(fittedStack, 'fitted stack'.getInfo())
...
print(vertexStack.getInfo(), 'vertex Stack'.getInfo())
...
print(chartBooleanMean.getInfo())
...
print(chartClusterMedian.getInfo())
