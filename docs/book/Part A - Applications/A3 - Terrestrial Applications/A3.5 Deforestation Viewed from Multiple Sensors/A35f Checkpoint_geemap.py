...
def func_dev(col, band):
    ...
    def func_kvy(col, band):
            ...
            return ee.ImageCollection(col.map(func_ofb
...
def func_seu(model, band):
        band = band + '_'

        # Function to extract a non-harmonic coefficients.

        def func_yxz(model, band, coef):
...
def func_pjz(data, modelImg, band, sensor):

        def func_nqt(img):
                return createPredImg(modelImg, img, band,
                sensor)

        return ee.ImageCollection(data.map(func_nqt


        ))

addPredicted = func_pjz
...
def func_wol(data, band):
        ...
        return ee.ImageCollection(data.map(func_swv
