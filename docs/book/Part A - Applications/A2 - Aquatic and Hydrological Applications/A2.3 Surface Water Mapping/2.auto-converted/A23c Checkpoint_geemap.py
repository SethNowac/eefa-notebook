...
dataCol = ee.List.sequence(0, x.length().subtract(1)).map(func_gix)
...
print('Adaptive threshold value:', localThreshold.getInfo())
...
dataCol = ee.List.sequence(0, x.length().subtract(1)).map(func_eom)
...
permanentWater = jrc.map(func_jto
