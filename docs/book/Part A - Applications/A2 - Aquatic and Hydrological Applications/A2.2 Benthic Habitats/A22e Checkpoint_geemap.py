...
def classify(img):
    ...
    arrayToDatatable = func_fck \
    .setOptions(
    pageSize = 15
    ),
    'rows: reference, 'cols': mapped')
    )
    print('Overall Accuracy', accuracyOA.getInfo())
    print('Users accuracy', accuraccyCons.getInfo())
    print('Producers accuracy', accuracyProd.getInfo())
    return classified
...
def rfbathymetry(img):
    ...
    print('R²', rSqr, 'RMSE', rmse.getInfo())
