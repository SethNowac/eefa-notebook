...
GRACE_yr = ee.ImageCollection.fromImages(years.map(func_pwi
...
def func_yux(feature):
    return ee.Image.cat(feature.get('primary'), feature.get(
    'secondary'))

GRACE_res_GLDAS = thirdJoin.map(func_yux)
...
GWa = ee.ImageCollection(GRACE_res_GLDAS.map(func_wfb
