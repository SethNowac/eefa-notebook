...
def func_tqn(dataImg, nodeIds, reducer):
    ...
    statsOut = ee.List(statsList).map(func_ihn)


    return ee.FeatureCollection(statsOut)

groupReduce = func_tqn
...
vectorData = sword.filterBounds(aoi).map(func_llg)
