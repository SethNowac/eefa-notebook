...
print(grip4_africaLength.limit(1).getInfo())
...
ee.Number(AlgeriaRoadLength.first().get('length')).getInfo())
...
countriesRoadLength.filter(ee.Filter.equals('country_na', 'Canada').getInfo()) \
.aggregate_sum('length'))
print('Length of all roads in France',
countriesRoadLength.filter(ee.Filter.equals('country_na', 'France').getInfo()) \
.aggregate_sum('length'))
...
# LGTM (nclinton). Reformatted. Added a setCenter# LGTM (nclinton). Reformatted. Added a setCenter.
Map