...
# ****************************************
util = require('projects/gee-edu/book:Part A - Applications/A1 - Human Applications/A1.8 Monitoring Gold Mining Activity Using SAR/modules/utilities_v1.1')
...
get_vvvh_asfRTC = func_gkt



# clip a list of images

def func_fcs(current,prev):
    imlist = ee.List(ee.Dictionary(prev).get('imlist'))
    geometry = ee.Dictionary(prev).get('geom')
    imlist1 = imlist.add(ee.Image(current).clip(geometry))
...
def pv(imList,p,median,j):
    ...
    P = ee.Image( util.chi2cdf(Z,f).multiply(one.subtract(omega2j)).add(util.chi2cdf(Z,f.add(4)).multiply(omega2j)) )
    # 3x3 median filter
...
def func_mlj(imList,significance,median):
    ...
    first1 = ee.Dictionary({'ell':1,'threshold':threshold,'cmap':cmap,'smap':smap,'fmap':fmap,'bmap':bmap})
