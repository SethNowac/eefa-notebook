...
def draw(text, pos, scale, props):
  ...
  if(props and "fontSize" in props):
    fontSize = props['fontSize']
  
  fontType = "Arial"
  if(props and "fontType" in props):
    fontType = props['fontType']
  
  glyphs = ee.Image('users/gena/fonts/' + str(fontType) + str(fontSize))

  if(props and 'resample' in props):
  ...
  if(props):
    props = {
      'textColor': props['textColor'] if 'textColor' in props else 'ffffff',
      'outlineColor': props['outlineColor'] if 'outlineColor' in props else '000000',
      'outlineWidth': props['outlineWidth'] if 'outlineWidth' in props else 0,
      'textOpacity': props['textOpacity'] if 'textOpacity' in props else 0.9,
      'textWidth': props['textWidth'] if 'textWidth' in props else 1,
      'outlineOpacity': props['outlineOpacity'] if 'outlineOpacity' in props else 0.4,
    }
...
def annotateImage(image, vis, bounds, annotations):
  ...
  def func_anno(annotation):
    ...
    annotation['format'] = annotation['format'] if 'format' in annotation else func_loc

    scale = annotation['scale'] if 'scale' in annotation else Map.getScale()
  ...
  imagesText = list(map((lambda x: func_anno(x)), annotations))
  
  images = [image] + imagesText

  if(vis):
      images = [image.visualize(vis)] + imagesText
...
def offsetToValue(offset, range):
  if(re.findall('px$', offset)):
    return ee.Number.parse(offset[0: len(offset) - 2])
  elif(re.findall('%$', offset)):
    offsetPercent = float(offset[0: len(offset) - 1])
