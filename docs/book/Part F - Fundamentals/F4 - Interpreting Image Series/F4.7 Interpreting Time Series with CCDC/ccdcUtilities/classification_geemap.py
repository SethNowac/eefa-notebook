...
import dates_geemap as dateUtils
import inputs_geemap as inputUtils
import ccdc_geemap as ccdcUtils
...
def makePhaseAmp(img, bandNames):
    ...
    phaseAmp1 =PhaseAmplitude(img, bandNames, '_SIN', '_COS')
    ...
    phaseAmp2 =PhaseAmplitude(img, bandNames, '_SIN2', '_COS2')
    ...
    phaseAmp3 =PhaseAmplitude(img, bandNames, '_SIN3', '_COS3')
...
if (trainProp):
    ...
    def classifySegments(imageToClassify, numberOfSegments, bandNames, ancillary, ancillaryFeatures, trainingData, classifier, studyArea, classProperty, coefs, trainProp, seed, subsetTraining):
        ...
        # Subset training data to studyarea if specified
if (studyArea && subsetTraining not == False):
            trainingData = trainingData.filterBounds(studyArea)
         else {
            trainingData = trainingData
        }
...
if (resultFormat == 'SegCollection'):
                ...
                def func_hwy(results, years, month, day):
                ...
                def func_czi(b):
                        ...
                        def func_ejf(i):
                            ...
                            return ee.List(coefs).map(func_ejf
                            return ee.String(b).cat('_').cat(i)

                predictors = ee.List(bandNames).map(func_czi
