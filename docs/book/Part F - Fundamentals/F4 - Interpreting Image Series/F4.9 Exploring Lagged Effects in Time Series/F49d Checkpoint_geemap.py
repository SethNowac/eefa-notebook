...
detrended = landsat8sr.map(func_phl)
...
def func_fxn(leftCollection, rightCollection, lagDays):
...
def func_lll(mergedCollection, band, lagBand):

    def func_pmt(
image):
            return image.toArray()

    return mergedCollection.select([band, lagBand]).map(func_pmt
...
sum30PrecipNDVI = ee.ImageCollection(lag30PrecipNDVI.map(func_vqs
...
merged34 = lagged34.map(merge).map(func_duq
...
fittedAR = merged34.map(func_uac)
...
fittedAR.select(['fitted', 'NDVI'].getInfo()) \
.mean(), 30) \
...
print(ui.Chart.image.series(
modeled, pt, ee.Reducer.mean(), 30.getInfo()) \
.setSeriesNames(['NDVI']) \
.setOptions(
title = 'Forecast',
lineWidth = 1,
