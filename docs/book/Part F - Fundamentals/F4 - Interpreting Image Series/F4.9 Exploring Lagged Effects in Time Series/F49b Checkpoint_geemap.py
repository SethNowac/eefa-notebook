...
detrended = landsat8sr.map(func_jkb)
...
def func_sqg(leftCollection, rightCollection, lagDays):
...
def func_tiu(mergedCollection, band, lagBand):

    def func_iws(
image):
            return image.toArray()

    return mergedCollection.select([band, lagBand]).map(func_iws
...
sum30PrecipNDVI = ee.ImageCollection(lag30PrecipNDVI.map(func_itg
