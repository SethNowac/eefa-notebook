...
detrended = landsat8sr.map(func_ssp)
...
def func_hcq(leftCollection, rightCollection, lagDays):
...
def func_xnx(mergedCollection, band, lagBand):

    def func_yuq(
image):
            return image.toArray()

    return mergedCollection.select([band, lagBand]).map(func_yuq
...
sum30PrecipNDVI = ee.ImageCollection(lag30PrecipNDVI.map(func_qba
...
merged34 = lagged34.map(merge).map(func_xkh
...
fittedAR = merged34.map(func_ccy)
