...
def func_cej(base, list):

    def func_ioi(i):
            return ee.String(base).cat(ee.Number(i).int())

    return ee.List(list).map(func_ioi)
...
fittedHarmonicPre = harmonicLandsatPre.map(func_wut)
...
fittedHarmonicDist = harmonicLandsatDist.map(func_dor)
...
mergedFitted.select(['fitted', 'NDVI'].getInfo()) \
.mean(), 30) \
.setOptions(
