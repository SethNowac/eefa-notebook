...
detrended = landsat8sr.map(func_pmb)
...
harmonicLandsat = landsat8sr.map(func_kfg)
...
fittedHarmonic = harmonicLandsat.map(func_ztu)
...
fittedHarmonic.select(['fitted', 'NDVI'].getInfo()) \
.mean(), 30) \
